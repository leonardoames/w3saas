...
const buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0", {
  variants: {
    variant: {
      ...
      outline: "border border-border bg-transparent text-foreground hover:bg-accent shadow-none",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({
  ...
  if (asChild || variant === "link") {
    ...
    return <Comp className={cn(buttonVariants({
      ...
      size,
      className
    }))} ref={ref} disabled={disabled} {...props}>{children}</Comp>;
  }
  ...
  return <motion.button ref={ref} className={cn("relative overflow-hidden", buttonVariants({
    ...
    size,
    className
  }), isDisabled && "pointer-events-none opacity-50")} onMouseEnter={e => {
    setIsHovered(true);
    ...
    scale: 0.98
      ...
      y: "100%"
        ...
        <AnimatePresence mode="popLayout">
          {isLoading && <motion.div key="loader" className="absolute inset-0 z-20 flex items-center justify-center" initial={{
        y: "100%"
      }} animate={{
        y: 0
      }} exit={{
        y: "-100%"
              ...
              <Loader2 className={cn("h-4 w-4 animate-spin", isHovered ? currentStyle.textHover : currentStyle.textInitial)} />
            </motion.div>}
        </AnimatePresence>

        <motion.span className={cn("relative z-10 flex items-center justify-center gap-2 whitespace-nowrap text-secondary-foreground", currentStyle.textInitial, isHovered && currentStyle.textHover)} animate={isLoading ? {
      opacity: 0
    } : {
      opacity: 1
    }} transition={transition}>
