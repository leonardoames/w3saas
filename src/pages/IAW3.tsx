      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Textarea placeholder="Ex: Crie uma descrição em HTML para um tênis esportivo feminino, focando em conforto e performance..." className="min-h-[300px] resize-none" value={prompt} onChange={e => setPrompt(e.target.value)} />
            <Button onClick={handleGenerate} className="w-full" size="lg" disabled={isGenerating}>
              <Sparkles className="mr-2 h-5 w-5" />
              {isGenerating ? "Gerando..." : "Gerar Conteúdo"}
            </Button>

            <div className="rounded-lg p-4 bg-[#e8e8e8]">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            {output && <Button variant="outline" size="sm" onClick={handleCopyHTML}>
          ...
          <CardContent>
            {output ? <div className="space-y-4">
              ...
              </div> : <div className="flex h-[400px] items-center justify-center text-muted-foreground">
